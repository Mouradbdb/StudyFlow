...
export async function POST(req: NextRequest) {
    ...
    let isCompleted = orderData.status === "COMPLETED" && orderData.purchase_units[0]?.payments?.captures?.[0]?.status === "COMPLETED";
    ...
    if (!isCompleted && orderData.status === "APPROVED") {
        ...
        if (!captureResponse.ok) {
            ...
            console.error("Capture failed for orderId:", orderId, "Status:", captureResponse.status, "Error:", errorText);
        ...
        isCompleted = captureData.status === "COMPLETED" && captureData.purchase_units[0]?.payments?.captures?.[0]?.status === "COMPLETED";
