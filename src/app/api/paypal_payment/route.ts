...
export async function POST(req: NextRequest) {
  ...
  if (!process.env.NEXT_PUBLIC_PAYPAL_CLIENT_ID || !process.env.PAYPAL_SECRET) {
    ...
    return NextResponse.json({ error: "Server configuration error: Missing PayPal credentials" }, { status: 500 });
  }
  ...
  try {
    ...
    if (!authResponse.ok) {
      ...
      return NextResponse.json({ error: "Failed to authenticate with PayPal: " + errorText }, { status: authResponse.status });
    }
    ...
    const orderResponse = await fetch("https://api-m.sandbox.paypal.com/v2/checkout/orders", {
      ...
      body: JSON.stringify({
        ...
        application_context: {
          return_url: `https://b147-41-142-75-61.ngrok-free.app/planner?success=true`,
          cancel_url: `https://b147-41-142-75-61.ngrok-free.app/planner?canceled=true`,
        },
      }),
    });
    ...
    if (!orderResponse.ok) {
      ...
      return NextResponse.json({ error: "Failed to create PayPal order: " + errorText }, { status: orderResponse.status });
    }
